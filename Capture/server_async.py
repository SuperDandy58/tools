...
def setup_server(description=None, context=None, cmdline=None):
    ...
    if not args.context:
        ...
        # If you initialize a DataBlock to addresses of 0x00 to 0xFF, a request to 0x100 will respond with an invalid address exception.
        # DataBlock をアドレス 0x00 ～ 0xFF に初期化すると、0x100 へのリクエストは無効なアドレス例外で応答します。
        # This is because many devices exhibit this kind of behavior (but not all)
        # これは、多くのデバイスがこの種の動作を示すためです (すべてのデバイスではありません)。
        if args.store == "sequential":
            ...
            # ギャップが存在する可能性があるスパース DataBlock を使用する
        ...
        if args.slaves:
            # The server then makes use of a server context that allows the server to respond with different slave contexts for different slave ids.
            # サーバーはサーバー コンテキストを使用して、サーバーが異なるスレーブ ID に対して異なるスレーブ コンテキストで応答できるようにします。
            # By default it will return the same context for every slave id supplied (broadcast mode).
            # デフォルトでは、指定されたすべてのスレーブ ID に対して同じコンテキストが返されます (ブロードキャスト モード)。
            # However, this can be overloaded by setting the single flag to False and  then supplying a dictionary of slave id to context mapping::
            # ただし、単一のフラグを False に設定し、スレーブ ID の辞書をコンテキスト マッピングに提供することで、これをオーバーロードできます。
            # The slave context can also be initialized in zero_mode which means that a request to address(0-7) will map to the address (0-7).
            # スレーブ コンテキストは、zero_mode で初期化することもできます。これは、アドレス (0 ～ 7) へのリクエストがアドレス (0 ～ 7) にマップされることを意味します。
            # The default is False which is based on section 4.4 of the specification, so address(0-7) will map to (1-8)::
            # デフォルトは False で、仕様のセクション 4.4 に基づいているため、アドレス (0-7) は (1-8) にマップされます。
            context = {
    ...
    # このフィールドまたは他のフィールドを設定しない場合、デフォルトで空の文字列が設定されます。
    # ----------------------------------------------------------------------- #
