...
**REMARK** It is recommended to use the async server! 
非同期サーバーを使用することをお勧めします
The sync server is just a thin cover on top of the async server and is in some aspects a lot slower.
同期サーバーは非同期サーバーの上にある薄いカバーにすぎず、いくつかの側面では非常に遅いです。
"""
import logging
    ...
    if args.comm == "tcp":
        ...
        server = StartTcpServer(
            ...
            # ignore_missing_slaves=True,  # ignore request to a missing slave
                                           # 行方不明のスレーブへのリクエストを無視する
            # broadcast_enable=False,  # treat slave_id 0 as broadcast address
                                       # ブロードキャストアドレスとしてslave_id 0を扱います
            # timeout=1,  # waiting time for request to complete
                          # リクエストが完了するまでの待ち時間
            # TBD strict=True,  # use strict timing, t1.5 for Modbus RTU
                                # Modbus RTU には厳密なタイミング (t1.5) を使用します。
        )
    elif args.comm == "udp":
